...
function deleteNoteFromfavorite($conn, $note_id) {
    // SQL statement to delete records
    $sql = "DELETE FROM favorite WHERE note_id = ?";
    // Prepare and bind parameters
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $note_id);
    // Execute the delete statement
    if ($stmt->execute()) {
        echo "Note deleted from favorite successfully";
        header("Location: dashboard.php");
        exit();
    } else {
        echo "Error deleting note from favorite: " . $conn->error;
    }
    // Close statement
    $stmt->close();
}
...
if(isset($_GET['note_id'])) {
    ...
    deleteNoteFromfavorite($conn, $note_id_to_delete);
} else {
